...
class KDTree(object):
    ...
    def find_nearest(self, X):
        ...
        return [_find_nearest(X_i, self.root, depth=0) for X_i in X]
...
def _find_nearest(X_i, node, depth):
    ...
    Returns:
        TreeNode: the node containing the nearest data point.
    ...
    best_nearest, best_dist, height = _find_nearest(X_i, next_node, depth + 1)

    if height <= 2:
        # Check siblings
        sibling_node = node.right if X_i[split_axis] < node_X[split_axis] else node.left
        sibling_nearest, sibling_dist, sibling_height = _find_nearest(X_i, sibling_node, depth + 1)
